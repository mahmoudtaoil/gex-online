...
function HomeContent() {
  ...
  return (
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
        <header style={{ background: 'linear-gradient(to bottom, rgba(2,6,23,0.95) 0%, rgba(2,6,23,0.6) 70%, transparent 100%)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '10px 12px' }}>
            {/* الصف الأول: لوجو + بحث يسار | واتساب + أدمن يمين */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', direction: 'ltr', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Link href="/" style={{ textDecoration: 'none', flexShrink: 0 }}>
                  {logoUrl? (<img src={logoUrl} alt="GEX" className="logo-img" style={{ height: 62, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 0 14px rgba(168,85,247,0.5))' }} />) : (<div style={{ fontFamily: 'system-ui', fontWeight: 900, fontSize: 26, letterSpacing: -1 }}>GEX<span style={{ background: 'linear-gradient(90deg,#38bdf8,#a855f7)', WebkitBackgroundClip: 'text', color: 'transparent' }}>X</span></div>)}
                </Link>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                ...
                <a href={settings?.whatsapp? `https://wa.me/${settings.whatsapp.replace(/[^0-9]/g,'')}` : '#'} target="_blank" style={{ padding: '7px 12px', borderRadius: 10, background: 'rgba(15,20,35,0.9)', border: '1.5px solid #22c55e', color: '#fff', display: 'flex', gap: 6, alignItems: 'center', fontSize: 13, fontWeight: 600, textDecoration: 'none', boxShadow: '0 0 12px rgba(34,197,94,0.3)' }}>
                  واتساب
                  <svg width="16" height="16" fill="#25D366" viewBox="0 0 24 24"><path d="M12 0a12 0 00-10 19l-2 5 5-2a12 0 1017-17A12 0 0012 0z"/></svg>
                </a>
              </div>
            </div>

            {/* الصف الثاني: أزرار الأقسام */}
            <nav style={{ display: 'flex', gap: 6, direction: 'rtl', flexWrap: 'wrap', justifyContent: 'center' }}>
              {cats.map(c => {
        ...
        <main style={{ maxWidth: 1536, margin: '0 auto', padding: '24px 12px', minHeight: '60vh' }}>
          {query? (
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <h1 style={{ fontSize: 32, fontWeight: 900, textShadow: '0 0 20px rgba(168,85,247,0.5)' }}>{current.t}</h1>
                <p style={{ color: '#9aa3b8', marginTop: 8, fontSize: 14 }}>شحن جميع {current.t} بأفضل الأسعار</p>
              </div>
          ...
          ) : (<div style={{ height: '40vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ textAlign: 'center' }}>{logoUrl? <img src={logoUrl} alt="logo" style={{ height: 140, margin: '0 auto 18px', filter: 'drop-shadow(0 0 25px rgba(168,85,247,0.6))' }} /> : <h1 style={{ fontSize: 48, fontWeight: 900, background: 'linear-gradient(90deg,#38bdf8,#a855f7)', WebkitBackgroundClip: 'text', color: 'transparent' }}>GEX ONLINE</h1>}<p style={{ color: '#9aa3b8', fontSize: 16 }}>اضغط على أيقونة البحث أو اختر قسم</p></div></div>)}
        </main>
      </div>

      <style>{`
        /* Grid متجاوب: 2 جوال | 4 تابلت | 5 لابتوب | 6 شاشة كبيرة */
    .gex-grid {
          display: grid;
          gap: 10px;
    ...
    .service-card:hover {
        ...
        /* تصغير اللوجو على الجوال شوي عشان ما يخرب الهيدر */
