...
function HomeContent() {
  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', background: '#020617', color: '#fff' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
        <header style={{ background: 'linear-gradient(to bottom, rgba(2,6,23,0.95) 0%, rgba(2,6,23,0.6) 70%, transparent 100%)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 50 }}>
          <div className="header-container" style={{ maxWidth: 1536, margin: '0 auto', padding: '10px 16px' }}>
            {/* الصف الأول: لوجو + بحث + واتساب + أدمن - دايماً صف واحد */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', direction: 'ltr', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Link href="/" style={{ textDecoration: 'none', flexShrink: 0 }}>
                  {logoUrl? (<img src={logoUrl} alt="GEX" style={{ height: 50, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 0 12px rgba(168,85,247,0.4))' }} />) : (<div style={{ fontFamily: 'system-ui', fontWeight: 900, fontSize: 25, letterSpacing: -1 }}>GEX<span style={{ background: 'linear-gradient(90deg,#38bdf8,#a855f7)', WebkitBackgroundClip: 'text', color: 'transparent' }}>X</span></div>)}
                </Link>
              ...
              {/* أزرار الأقسام - عالديسكتوب بنفس الصف، عالجوال تنزل تحت */}
              <nav className="header-nav" style={{ display: 'flex', gap: 8, direction: 'rtl', justifyContent: 'center', flex: '1 1 auto' }}>
                {cats.map(c => {
    ...
    .service-card:hover {
        ...
        /* عالديسكتوب: الهيدر صف واحد كبير */
        @media (min-width: 1024px) {
         .header-nav button {
        ...
        /* عالجوال والتابلت: الأزرار تنزل تحت اللوجو والواتساب */
